...
features:
  - type: line
    ...
    shorten: false
    traverse: false
    dashedStart: false  # optional; renders first 30px as dashed line
    dashedEnd: false    # optional; renders last 30px as dashed line
  - type: rappel
    id: 2
    x: 150
    y: 150
  ...
  - type: access
    ...
    text: 'Exit here'   # optional label text; supports multiline with \n
    textOffsetX: 0      # optional; drag the text independently
    textOffsetY: 0      # optional
```
...
- Access features have optional draggable text labels via `text`, `textOffsetX`, `textOffsetY` fields. Text is positioned at the midpoint of the access line and can be moved independently. Text color matches the access type (green for entrance, black for exit). Access labels support multiline text via `\n` characters; rendered using `<tspan>` elements with 14px line height. The `updateAccess()` method rebuilds tspans on every update to maintain multiline support (similar to `updateRappel()` and `updateNote()`).
- **Multiline text update pattern**: For features with draggable multiline text (rappel descriptions, note text, access labels, anchor names), the `updateXxx()` methods must rebuild the `<tspan>` children rather than using `textContent`, which would replace all child elements. The pattern is: `textEl.innerHTML = ''; text.split('\n').forEach((line, i) => { /* create tspan */ })`. This ensures multiline text persists correctly when features are moved or updated.
...
- Line dashed segments: `dashedStart` and `dashedEnd` boolean fields control rendering of dashed line segments at the start/end of a line. When enabled, a 30px dashed segment is rendered (using `stroke-dasharray: '5,5'` and `stroke-linecap: 'butt'`), and the solid middle segment is shortened accordingly. The line is split into three separate SVG `<line>` elements with distinct classes (`dashed-segment-start`, `line-shape`, `dashed-segment-end`) to enable selective updates via `updateLine()` in editor-features.js.
...
- **Box select / group move**: toolbar "Select" button (dashed-rect icon) toggles `selectMode`. In select mode, dragging empty canvas draws a selection box; features whose primary point falls inside are selected. Without shift: box selection *replaces* the current selection. With shift held: box selection *adds* to the current selection (additive). Shift+click a single feature toggles it in/out of the selection. Dragging any selected feature moves all of them together (grid-snapped; undo-able). ESC clears selection; second ESC exits select mode. Selection highlights (blue dashed rects) are drawn in the cursor layer by `renderSelectionHighlights()`, which is called from `render()` so they survive full redraws. `applySnapshotWithOffset()` offsets both `x`/`y` and `connectionX`/`connectionY` (present on anchors) to keep the snap point aligned. Implementation tracks `boxSelectShiftHeld` to preserve shift state from `startBoxSelect()` through `endBoxSelect()`, enabling correct additive behavior even when the shift key is released during dragging.
