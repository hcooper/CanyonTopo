...
export class Draw {
      ...
      switch (feature.type) {
        case "line":
          this.plot_line(
            ...
            feature.slope || DEFAULT_LINE_SLOPE,
            feature.shorten || DEFAULT_LINE_SHORTEN,
            feature.label || null,
            feature.traverse || false,
  ...
  add_label(label, x_offset, y_offset, textStyle = "", angle = 0) {
    const font_size = DEFAULT_FONT_SIZE;
    var label_x;
    var label_y;
    let est_width = label.length * DEFAULT_FONT_SIZE * 0.6;

      console.log(label, x_offset, y_offset, angle);


    if (x_offset && y_offset) {
      // The label has been manually placed
      console.log("provided", x_offset, y_offset);
      label_x = this.current_x + x_offset;
      label_y = this.current_y + y_offset;
    } else {
      // Track the rotation of the feature to better place the label

      // Placement for a 0-degree line
      y_offset = -40;
      x_offset = 0;
      ...
      const rotated_x = x_offset * Math.cos(radians) - y_offset * Math.sin(radians);
      const rotated_y = x_offset * Math.sin(radians) + y_offset * Math.cos(radians);

      // Final label position
      label_x = this.current_x + rotated_x;
      label_y = this.current_y + rotated_y;
  ...
  plot_line(
    ...
    if (label) this.add_label(label, labelOffsetX, labelOffsetY, textStyle, slope);
  ...
  plot_rappel(
    ...
    // If sections are provided, use multi-section approach
    if (sections && Array.isArray(sections) && sections.length > 0) {
      if (label) this.add_label(label, labelOffsetX, labelOffsetY, textStyle, sections[0].slope);
