...
export class Draw {
  ...
  plot_line(
    ...
    if (sections && Array.isArray(sections) && sections.length > 0) {
      this.plot_multi_section_line(sections, shorten, label, traverse, labelOffsetX, labelOffsetY, textStyle, id);
      return;
    }
  ...
  plot_multi_section_line(sections, shorten, label, traverse, labelOffsetX, labelOffsetY, textStyle, id) {
    const start_x = this.current_x;
    const start_y = this.current_y;
    ...
    if (label) {
      // Position label at the midpoint of the entire multi-section line
      const mid_x = (start_x + this.current_x) / 2;
      const mid_y = (start_y + this.current_y) / 2;
      this.add_label(label, labelOffsetX + (mid_x - start_x), labelOffsetY + (mid_y - start_y), textStyle);
    }
  ...
  plot_rappel(
    ...
    // If sections are provided, use multi-section approach
    if (sections && Array.isArray(sections) && sections.length > 0) {
      this.plot_multi_section_rappel(sections, label, labelOffsetX, labelOffsetY, textStyle, rappelId);
      return;
    }
    ...
    const parallel_offset = -15; // Distance between lines
  ...
  plot_multi_section_rappel(sections, label, labelOffsetX, labelOffsetY, textStyle, rappelId) {
    const start_x = this.current_x;
    const start_y = this.current_y;
    let path_data = `M ${this.current_x} ${this.current_y}`;
    ...
    sections.forEach((section, sectionIndex) => {
      const length = (section.length || DEFAULT_LINE_LENGTH) * VERT_SCALE;
      const slope = section.slope || DEFAULT_RAP_SLOPE;
      const slope_radians = (slope * Math.PI) / 180;
      ...
      // For the parallel path (arrow line), calculate offset for each segment
      ...
      if (sectionIndex === 0) {
        const px1 = this.current_x + offset_x;
        const py1 = this.current_y + offset_y;
        parallel_path_data = `M ${px1} ${py1}`;
      }
      
      const px2 = x2 + offset_x;
      const py2 = y2 + offset_y;
      parallel_path_data += ` L ${px2} ${py2}`;
    ...
    if (label) {
      // Position label at the midpoint of the entire multi-section rappel
      const mid_x = (start_x + this.current_x) / 2;
      const mid_y = (start_y + this.current_y) / 2;
      this.add_label(label, labelOffsetX + (mid_x - start_x), labelOffsetY + (mid_y - start_y), textStyle);
    }
