import {scrollToItem} from './dragging.js';

export function addHightlightEvents() {
  // svg -> list
  const svgElements = document.querySelectorAll(["text", "line", "path"]);
  svgElements.forEach((element) => {
    element.addEventListener("mouseover", function () {
      if (window.topo.highlight_mode) {
...
export function removeHighlightFromSVGItem() {
  const listItem = document.querySelector(`.topo-highlight-svg`);
  if (listItem) {
    listItem.classList.remove("topo-highlight-svg");
  }
}

export function highlightSVGItem(index) {
