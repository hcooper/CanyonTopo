...
export function saveChangesToMediaWikiAPI(name, newYamlContent) {
  if (!name || typeof name !== 'string') {
    alert('Invalid page name provided');
    return;
  }
  
  if (!newYamlContent || typeof newYamlContent !== 'string') {
    alert('Invalid YAML content provided');
    return;
  }
  
  if (!window.topo.RW_DOMAIN) {
    alert('RW_DOMAIN is not configured');
    return;
  }
